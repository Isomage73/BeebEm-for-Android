...
bool TeletextHalfMode = false; // set true to force TeletextStyle=2 (half-scanline mode) for all teletext
int CurY=-1;
...
static void DoMode7Row(void) {
  ...
  for (CurrentChar = 0; CurrentChar < CRTC_HorizontalDisplayed; CurrentChar++) {
    ...
    // On Android, always use TeletextStyle=1 (all 20 scanlines per character).
    // TeletextStyle=2 renders only odd scanlines with alternating black rows —
    // a CRT interlace simulation that looks flickery on modern phone displays.
    TeletextStyle = TeletextHalfMode ? 2 : 1;
...
// Returns the number of pixel rows actually rendered by the CRTC in the
// most recently completed BBC frame.  Used by the Android platform layer
// to clip the GL texture to the active picture area, eliminating stale
// buffer content from previous screen modes.
int GetVideoRenderedHeight()
{
    int h = VideoState.PreviousLastPixmapLine + 1;
    if (h <= 0 || h > BEEBEM_BITMAP_HEIGHT) return BEEBEM_BITMAP_HEIGHT;
    return h;
}
