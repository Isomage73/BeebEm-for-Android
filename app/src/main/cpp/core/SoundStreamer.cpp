...
SoundStreamer *CreateSoundStreamer(int samplerate, int bits_per_sample, int channels)
{
	...
	// On macOS, sound is driven by CoreAudio pulling from the ring buffer
	// (see CoreAudioStreamer.mm). SoundStreamer is not used on this path.
	...
	// Linux: sound is handled via AddBytesToSDLSoundBuffer / SDL_OpenAudio.
	UNREFERENCED_PARAMETER(samplerate);
	UNREFERENCED_PARAMETER(bits_per_sample);
	UNREFERENCED_PARAMETER(channels);
