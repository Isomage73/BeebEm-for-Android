...
BeebWin::BeebWin()
{
	...
	// On Android, HOME is set to filesDir by JNI before construction.
	// Assets are extracted flat into filesDir, so use it directly.
	{
		const char *HomePath = getenv("HOME");
		if (HomePath != nullptr) {
			snprintf(m_UserDataPath, MAX_PATH, "%s/", HomePath);
		} else {
			m_UserDataPath[0] = '\0';
		}
	}
...
bool BeebWin::Initialise()
{
	...
	// Font lives in UserData/ (extracted from assets on Android, copied on macOS).
	...
	// Re-anchor timing at the first StartOfFrame() call; without this,
	// m_TickBase is set during LoadPreferences() and any startup delay
	// (GL thread creation, asset extraction) permanently skews UpdateTiming()
	// into "behind" mode, causing it to skip all but 1-in-100 BBC frames.
	m_LastTickCount = 0;
...
void BeebWin::CreateBitmap()
{
	...
	#elif defined(__APPLE__) || defined(BEEBEM_ANDROID)

	m_screen = (char*)g_videoBuffer;
	m_screen_blur = (char*)calloc(BEEBEM_BITMAP_WIDTH * BEEBEM_BITMAP_HEIGHT, 1);

	// Identity mapping: VideoULA_Palette already stores (ULA_write ^ 7), which
	// maps physical BBC colors to 0-7 with bit0=R, bit1=G, bit2=B — matching
	// both teletext (doHorizLine writes 0=black, 7=white directly) and the
	// Android palette (palette[i] built from bits 0-2 as R,G,B).
	for (int i = 0; i < 256; ++i) cols[i] = (unsigned char)i;
...
{
	#ifdef BEEBEM_ANDROID
	// Wall-clock pacing already lives in nativeRunFrame on Android, so the
	// core-side UpdateTiming() frame-skipping is redundant — and worse, when
	// it decides to skip a frame it short-circuits VideoAddCursor + UpdateLines,
	// so CaptureFrame() never fires and the GL keeps redrawing the last
	// captured frame indefinitely. (User-visible symptom: screen freezes after
	// a mode change; opening the menu for >1s triggers UpdateTiming's reset
	// path which "fixes" it.)  Always render every BBC frame on Android.
	(void)UpdateTiming();  // still call it so stats counters update
	return true;
	#else
